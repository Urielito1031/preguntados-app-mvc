...

{
   ...
   public function getPorcentajeAciertosByPlayer():array{
    ...
    public function saveTokenForValidationByUser(int $token, int $id_user): void
    {
    ...
    public function findTokenByIdUser(int $id_user): int {
        ...
        try {
            ...
            $stmt->execute();
            $tokenByUser = (int)$stmt->fetchColumn();
    ...
    public function validateAccountByUserId (int $id_user) : void {
        $query = "UPDATE usuario SET cuenta_validada = 1 WHERE id_usuario = :id";
        ...
        try {
            ...
            throw new PDOException("Error al validar la cuenta del usuario: " . $e->getMessage());
        }

    }

    public function validateAccountRequestByUserId (int $id_user) : bool {
        $query = "SELECT cuenta_validada FROM usuario WHERE id_usuario = :id";

        try {
            $stmt = $this->conn->prepare($query);
            $stmt->bindValue(':id', $id_user, PDO::PARAM_INT);
            $stmt->execute();
            $cuenta_validada = (bool)$stmt->fetchColumn();
